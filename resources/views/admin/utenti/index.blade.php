                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    ...
                                    <th scope="col" class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Nome Proprietario</th>
                                    ...
                                    <th scope="col" class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Telefono</th>
                                    ...
                                    <th scope="col" class="px-3 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Ord. Chiamata</th>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @forelse ($utenti as $utente)
                                    <tr>
                                        ...
                                        <td class="px-4 py-3 text-sm font-medium">{{ $utente->nome_proprietario ?? '-' }}</td>
                                        <td class="px-4 py-3 text-sm font-medium">{{ $utente->email }}</td>
                                        <td class="px-4 py-3 text-sm font-medium">{{ $utente->phone_number ?? '-' }}</td>
                                        ...
                                        <td class="px-4 py-3 text-sm font-medium text-center">{{ $mappaOrdine[$utente->id] ?? '-' }}</td>
                                        <td class="px-4 py-3 text-sm font-medium">
                                            <div class="flex items-center space-x-4"> {{-- Aumentato lo spazio per le icone --}}
                                                
                                                {{-- ICONA MODIFICA --}}
                                                <a href="{{ route('admin.utenti.edit', $utente->id) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-500" title="Modifica">
                                                ...
                                                {{-- ICONA ELIMINA --}}
                                                <form action="{{ route('admin.utenti.destroy', $utente->id) }}" method="POST" onsubmit="return confirm('Sei sicuro di voler eliminare questo utente? Questa azione è irreversibile.');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="text-red-600 dark:text-red-400 hover:text-red-700 dark:hover:text-red-500" title="Elimina">
                                                ...
                                                @if ($utente->phone_number)
                                                    @php
                                                        ...
                                                        $numeroWhatsAppFinal = preg_replace('/[^0-9]/', '', $utente->phone_number);
                                                        
                                                        $loginUrl = route('login');

                                                        $testoBaseMessaggio = "Ciao " . $nomeSalutare . "!\n\n" .
                                                                                      "Puoi accedere alla tua squadra Fantastazione (" . $utente->name . ") su " . $loginUrl . " con:\n\n" .
                                                                                      "Email: {$utente->email}\n" .
                                                                                      "Password: Fantastazione\n\n" .
                                                                                      "Ti consigliamo di cambiarla al primo accesso. Se non la ricordi, usa l'opzione 'Password dimenticata?'.";
